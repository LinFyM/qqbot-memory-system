...
class RecallMemoryTrainer:
    ...
    def _prepare_model_once(self):
        ...
        if not self._model_prepared:
            ...
            # 对于单GPU配置，确保模型在正确的设备上（Accelerator可能会自动分配）
            if not self.use_auto_device and not self.multi_gpu_list:
                first_param = next(self.model.parameters())
                current_device = first_param.device
                if current_device != self.primary_device:
                    if self.is_main_process():
                        print(f"   ⚠️ 模型在 {current_device}，移动到 {self.primary_device}")
    ...
    def _freeze_embeddings_except_special_tokens(self):
        ...
        # 构建行级梯度掩码：只允许<recall>更新，其余（包括<|memory_pad|>）屏蔽
        ...
        row_mask[self.recall_token_id] = True
    ...
    def cleanup(self):
        ...
        try:
            ...
            if hasattr(self, 'merged_model') and self.merged_model is not None:
                try:
                    # 检查是否真的是多GPU模型（需要检查device_map是否包含多个设备）
                    is_multi_gpu = False
                    if hasattr(self.merged_model, 'hf_device_map') and self.merged_model.hf_device_map:
                        # 检查device_map中是否有多个不同的设备
                        device_map = self.merged_model.hf_device_map
                        if isinstance(device_map, dict):
                            ...
                            for key, value in device_map.items():
                                if isinstance(value, (int, str)):
                                    ...
                                    devices.update(str(v) for v in value.values() if isinstance(v, (int, str)))
    ...
    def _train_model(self, train_texts, train_embeddings, val_texts, val_embeddings,
        ...
        for epoch in range(num_epochs):
            ...
            # 创建进度条
            if self.is_main_process():
                progress_bar = tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}", disable=False)
            else:
            ...
            for batch in progress_bar:
                ...
                # 更新进度条
                if self.is_main_process() and train_steps > 0:
                    avg_loss = train_loss / train_steps
                    progress_bar.set_postfix({'loss': f'{avg_loss:.6f}', 'step': train_steps})
            ...
            # 创建验证进度条
            if self.is_main_process():
                val_progress_bar = tqdm(val_loader, desc=f"验证 {epoch+1}/{num_epochs}", disable=False)
            else:
            ...
            with torch.no_grad():
                for batch in val_progress_bar:
                    ...
                    # 更新验证进度条
                    if self.is_main_process() and val_steps > 0:
                        avg_val_loss_so_far = val_loss / val_steps
                        val_progress_bar.set_postfix({'val_loss': f'{avg_val_loss_so_far:.6f}'})
